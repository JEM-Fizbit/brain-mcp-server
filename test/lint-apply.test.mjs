...
const { applyLintFixes } = await import(
...
test("applyLintFixes runs all four fixes and writes changed files", async () => {
  ...
  assert.deepEqual(summary.orphansIndexed, ["07_orphan.md"]);
  assert.equal(summary.tasksRelocated.length, 1);
  assert.equal(summary.doneStamped.length, 2); // undated + newly-relocated
