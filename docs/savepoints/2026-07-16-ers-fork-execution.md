...
**Status:** execution checkpoint — Phase 0/1 complete; Phase 2 private `v1.4.3` overlay committed and pushed; corrective `v1.4.5` intake/rebase is the next repository gate; M1 Fly app reserved but not deployed
...
Execution is continuing from a stable private `v1.4.3` overlay baseline. The next repository mutation is to import annotated tag `v1.4.5`, reapply the unchanged four-surface overlay, run the private gates, and push a release-specific branch after its individual approval. The ERS Fly app name has been reserved in the ERS organisation but has no image or deployment. Supabase organisation custody/consolidation remains an external administration step. No deployment, migration, seed, secret, DNS, OAuth, or Brain-data mutation is authorized by this savepoint update. Never mirror `main` HEAD.
...
| Other ERS infra | ERS Fly organisation/admin/billing path exists and empty app `ers-brain-mcp` is reserved with no image or deployment. Supabase organisation custody/consolidation is in progress. No OAuth, DNS, database, migration, seed, secret, or data-cutover action is recorded here |
