...
**Status:** execution checkpoint — Phase 0/1 complete; Phase 2 private `v1.4.5` overlay committed and pushed; personal/ERS Supabase custody recovery complete; M1 Fly app reserved but not deployed; final ERS hostname live in DNS; GitHub OAuth-app creation is the next approval gate
**Repo:** `/Users/johnemilad/Projects/brain-mcp-server` (public; branch `main`; private overlay branch `ers/v1.4.5` at `2cee69e`, based on exact annotated upstream tag `v1.4.5` / commit `806117b`)
...
Execution is continuing from a stable private `v1.4.5` overlay baseline. The ERS Fly app name is reserved in the ERS organisation but still has no image, deployment, machine, IP address, or secrets. Supabase custody recovery and personal-clone cleanup are complete as described below. The final hostname `brain.ersgenomics.online` is now authoritative in Cloudflare DNS and registered on the Fly app. The next hosted mutation is creation of the GitHub OAuth app under `ERS-Genomics`; it has **not** been approved or performed. No further deployment, migration, seed, secret, OAuth, or Brain-data mutation is authorized without its individual approval. Never mirror `main` HEAD.
...
| ERS mirror repository | Verified private and under ERS custody; exact annotated `v1.4.5` imported (tag object `79160f3`, peeled commit `806117b`); branch `ers/v1.4.5`; exact four-surface overlay committed at `2cee69e` and pushed to private `origin`; no secrets committed |
| Other ERS infra | ERS Fly organisation/admin/billing path exists and empty app `ers-brain-mcp` is reserved with no image, release, machine, IP, or secrets. The ERS Supabase destination remains under ERS custody; the completed personal recovery clone is isolated in the personal Pro organisation. The final hostname has been registered on Fly and added to Cloudflare DNS. No GitHub OAuth app, migration, seed, runtime secret, or ERS deploy has occurred |
| Phase 2 approval | Repository/mirror intake and hostname DNS are complete. The next ask-first hosted action is creation of the ERS-Genomics GitHub OAuth app using the final homepage and callback recorded below |
| Private overlay | Pushed at `2cee69e`: ERS registry, `fly.toml`, deployment expectations, and ERS deploy/ops runbook only; verified zero divergence from `v1.4.5` across `src`, `db`, `scripts`, `package.json`, `package-lock.json`, and `Dockerfile` |
...
**Phase 2 — private `v1.4.5` overlay and final hostname complete; M1 stand-up continuing:** the private remote contains exact annotated tag `v1.4.5` and branch `ers/v1.4.5` at overlay commit `2cee69e`. The ERS Fly app is reserved, its final hostname is authoritative in DNS, and its certificate is registered but pending the first deployment. The next individually ask-first M1 step is GitHub OAuth-app creation. Every later M1 mutation remains separately ask-first.
...
## Exact restart gate (2026-07-20)

Stop state: no hosted action is currently running. Cloudflare browser work is complete. Both DNS records are saved and externally visible. The public checkout and private mirror were clean and synced immediately before this savepoint update.

The next session must:

1. Read this savepoint first, then read spec 012 in full. Do not reopen decisions resolved in spec §9.
2. Run read-only checks only: public and private `git status`, public/private remotes, and `FLY_CONFIG_DIR=/Users/johnemilad/.config/fly-ers fly certs check brain.ersgenomics.online -a ers-brain-mcp`.
...
4. Present the next mutation for approval: create the GitHub OAuth app under `ERS-Genomics` with homepage `https://brain.ersgenomics.online` and callback `https://brain.ersgenomics.online/authorize/github/callback`. The runtime requests `read:user user:email`.
5. After approval, verify the displayed GitHub identity is `jemilad-ers` and the owner is `ERS-Genomics` before saving the app. The client secret must be user-custodied in Dashlane and entered directly; never print, paste into chat, or commit it.
6. Return to ordered M1 steps after the OAuth app: apply the six tenant-neutral migrations to the existing ERS destination with advisors after each and the full security gate after the sequence; create the `brain_runtime` login; verify the private bucket; prepare ERS registry/seed; set fresh Fly secrets; deploy the pinned private release; run empty-stack smokes; register assets. Every mutation is a separate ask-first gate.

Fresh-session kickoff prompt:

> Resume the ERS Brain MCP fork (spec 012). Read `docs/savepoints/2026-07-16-ers-fork-execution.md` first, then `docs/specs/012-ers-mcp-fork.md`, then `docs/TOOLING.md`. Treat the savepoint as current state and the spec as the plan of record; do not re-litigate §9. Start with read-only cleanliness, remote, Fly-app, DNS, and certificate checks. The private `v1.4.5` overlay and Cloudflare DNS are complete. The Fly certificate may remain pending until first deploy because the empty app has no address. The next unapproved hosted mutation is creation of the GitHub OAuth app under `ERS-Genomics` using homepage `https://brain.ersgenomics.online` and callback `https://brain.ersgenomics.online/authorize/github/callback`; present that one step for approval and stop. Preserve the public/private boundary, use the isolated ERS Fly profile, and ask first for every ERS-infra or hosted mutation.
