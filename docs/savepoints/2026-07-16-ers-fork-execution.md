...
**Status:** overlay-preflight checkpoint — Phase 0/1 and Phase 2 Steps 1–2 complete at `v1.4.2`; generic overlay blockers repaired upstream at `v1.4.3`; paused before private `v1.4.3` intake and overlay push
**Repo:** `/Users/johnemilad/Projects/brain-mcp-server` (public; branch `main`; private baseline `v1.4.2`; next exact mirror candidate `v1.4.3`)
...
| Public upstream | `v1.4.3` is the exact annotated overlay-capable mirror candidate; closeout requires clean `main`, `origin/main`, and tag convergence |
...
| Phase 2 approval | The approved `v1.4.2` Step 3 preflight exhausted without a push because its mandatory gates failed; the next ask-first action is exact `v1.4.3` intake plus the four-surface private overlay |
...
The first imported post-structural mirror target was `v1.4.2`, not the historical `v1.2.0` anchor and not `main` HEAD. The private overlay must now move to annotated `v1.4.3` because its generic release-contract fixes are required to satisfy the spec's own acceptance gates.
...
**Phase 1 + structural integration — complete:** §1 P0/P1 upstream hardening shipped test-first at `v1.2.0`; spec 013 runtime/content changes were deployed through `v1.4.1`; `v1.4.2` was the first imported tag; `v1.4.3` is the overlay-capable release candidate.

**Phase 2 — paused after Step 3 preflight:** the private remote still contains only annotated tag `v1.4.2` and its reachable history. Importing exact tag `v1.4.3`, rebasing the local four-surface overlay onto it, running the full private gates, and pushing the release branch is the next individually ask-first action. Every M1 step remains separately ask-first.
...
Node 22: `npm run build` · `npm test` · `npm run eval:brain:routing`; verify package/tag agreement, `git diff v1.4.2..v1.4.3 -- src db Dockerfile fly.toml` is empty, and the only script delta is the generic guarded-deploy overlay support; per-phase gates are embedded in spec §§3–7; docs checks include `git diff --check` and Markdown-link verification.
