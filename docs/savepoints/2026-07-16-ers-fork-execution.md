...
| Personal stack | Fly runtime `jem-brain-mcp` release `v52` serves server `v1.4.5` and still points to the original mixed Supabase project through the transaction pooler. The live connector therefore still exposes both `ai-brain-jem` and `ers-brain`. The personal clone is under personal organisation custody; its 70 JEM Storage objects / 13,756,608 bytes passed source-to-destination SHA-256 verification, its exact two-revision post-backup database delta passed parity, and its ERS relational tenant has been removed with JEM/OAuth integrity guards. The clone database now registers only `ai-brain-jem`; 128 ERS Storage catalogue rows remain pending separate deletion. No Fly secret, OAuth, endpoint rewire, ERS source-project prune, or runtime cutover has occurred. The existing runtime remains live while Storage/OAuth are resolved and the clone is rewired and soaked |
...
**Phase 3 — recovery plus M2/M3 (spec §§4–6, checklist §10):** the completed database clone has been transferred to the personal Supabase organisation; its JEM-only Storage copy and post-backup database delta passed parity, and its ERS relational tenant has been removed. Next delete the remaining 128 ERS Storage catalogue/object entries, migrate or deliberately re-enrol the personal OAuth state, and verify/soak the rewired `jem-brain-mcp`. In parallel-safe ERS work, finish the private `v1.4.5` intake and stand up `ers-brain-mcp` against the current ERS Supabase project. Then re-seed/verify ERS (current md baseline = 50 but the live SharePoint count on the day is authoritative; sources `BRAIN_EXPECTED_SOURCE_COUNT=125`, never 128), run parity + restore rehearsal, re-point ERS sync and clients, soak 2–3 days, and only then prune JEM data from ERS — **CEO sign-off on the verification evidence first**; write the OAuth-row purge predicate before cutover day.
