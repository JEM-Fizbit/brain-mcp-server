...
The earlier inspection of plugin settings and John's fresh-task report both correctly showed the older installed schema before this refresh. Those snapshots no longer describe the fresh clients tested after refresh. The running desktop task can still retain its original declarations; a fresh desktop task remains separate acceptance evidence.
