...
**Resolved on 11 September 2026:** both existing Brain definitions were refreshed through **Workspace settings → Apps → selected Brain → App details → Actions / See details → Refresh**. The separate Plugins configuration panel did not expose that control. Fresh ChatGPT and Codex CLI sessions now expose `brain_prepare_ingest` and `brain_update_file.expected_revision`, and actual describe/status/preflight calls passed independently for both Brains.

The earlier inspection of plugin settings and John's fresh-task report both correctly showed the older installed schema before this refresh. Those snapshots no longer describe the fresh clients tested after refresh. The original running desktop task retains its old declarations. A separately authorized fresh Codex desktop task subsequently exposed both current markers and passed actual preflight plus Owner creation, reviewed replacement, stale refusal and cleanup on both Brains. Existing lower-role client journeys remain separate acceptance work.

No working connection was deleted or re-enrolled, and no workforce grant or manual approval policy was changed. Preserve reviewed-revision enforcement. Real Owner writes, stale refusal, cleanup and lower-role journeys must be recorded per client; schema discovery alone does not close acceptance. A host safety refusal must be distinguished from a server authorization/stale-revision refusal and must not be bypassed. Use the verified [metadata refresh procedure](protocols/OPENAI_MCP_CONNECTOR_RECOVERY.md#metadata-only-refresh--verified-11-september-2026); OAuth-state recovery is for demonstrated authentication failures.
