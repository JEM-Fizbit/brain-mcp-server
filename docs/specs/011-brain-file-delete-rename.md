...
**Status:** SHIPPED (2026-07-10) — built inside-out under strict TDD across 6 units (data model → 3 store backends → service layer → tools → guarded sync → e2e + docs); 286 tests green. **All three operator Hard Gates complete:** (1) tombstone migration applied to the live personal Supabase (`brain_file_revisions`: `deleted` NOT NULL default false, `content`/`content_sha256` nullable, presence check constraint — validated against 609 existing rows); (2) server deployed to Fly (`jem-brain-mcp` v43, `/health` → v1.2.0); (3) first live delete = registry action A11 (the stale `ers-brain/ip_landscape.md` redirect stub tombstoned via the production code path, canonical `ip/landscape.md` intact, recoverable via `brain_restore_file`). Prior status: DRAFT v2 (2026-07-08), revised against two adversarial reviews.
