...
**Status:** IMPLEMENTED (2026-07-10) — built inside-out under strict TDD across 6 units (data model → 3 store backends → service layer → tools → guarded sync → e2e + docs). All non-hosted work landed and verified (286 tests green; Postgres store tests build-verified/env-gated). **Remaining = operator Hard Gates only:** live schema migration, Fly deploy, and the first live delete (registry action A11) — see Rollout. Prior status: DRAFT v2 (2026-07-08), revised against two adversarial reviews.
