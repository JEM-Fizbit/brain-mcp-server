...
- **John/operator menu-bar app:** proceed. Shows helper health, last sync, conflicts, doctor status, restart/open logs/open cockpit actions. It owns the local sync watcher and cockpit server as child processes for the consolidated John/operator stack.
...
`npm run sync:menubar:launchd:plist`. The app now owns the local sync watcher
and cockpit server as child processes, reports a `brain-monitor-stack.json`
supervisor status file for doctor, and keeps doctor/log/cockpit actions in one
menu-bar surface for John/operator use. Signed/notarized distribution and any
colleague-facing tray/menu-bar product remain deferred.
