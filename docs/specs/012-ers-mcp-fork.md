...
|---|---|---|---|
| 4 | John's ERS registry identity: `jemilad-ers` vs `JEM-Fizbit` (220941196), and role (owner vs admin). Docs currently contradict each other; the jemilad-ers numeric id is **not yet looked up** | §3.6 | Lean `jemilad-ers` for exit hygiene; verify org membership + OAuth-app policy first |
...
GATE 0  □ Decisions 1–16 (§9) recorded  □ owners+billing named  □ hostname locked
        □ jemilad-ers (or chosen) numeric id fetched  □ purge predicate for OAuth rows written
