...
| Supabase | `brain-platform-pilot` ref `omnwbcdtmtvxasgdmvwr`, "ERSG Prototypes" org (**personal-owned despite the name**), eu-west-2, Pro; PITR off | New ERS org + project; 9 migrations in order (including durable OAuth state, tombstones, private revision FTS, bounded observability, source-reference identity, and Entra access grants); `brain_runtime` login on transaction pooler `:6543`; bucket `brain-artifacts`; security re-gate | ~1–2 d |
...
**Execution status (2026-07-20):** P0 and P1 completed upstream with test-first coverage at `v1.2.0`. The generic spec 013 server and Brain-structure content baseline shipped at `v1.4.1`; `v1.4.2` reconciled the fork runbook, complete migration manifest, structural data baseline, and regression coverage and became the first private-mirror import. The first real overlay preflight then exposed two generic release-contract gaps: the deployment test still named both pilot Brains, and the exact-tag deploy guard could not attest a committed overlay. `v1.4.3` fixed both upstream without changing `src/`, `db/`, the runtime image, or the resolved topology; the exact four-surface ERS overlay was then committed and pushed privately at `adf6b3f`. The 2026-07-18 beta incident proved that ordinary Brain writes also needed a reserved external-namespace guard and that repeated identical conflicts needed an idempotency boundary; `v1.4.4` fixes both without schema change. Deployed `v1.4.5` closes the related local-state maintenance gap with a parity-gated, backup-first rebase command and is the required base for the next private-mirror intake before first deployment.
...
4. **Migrations, in file order (all ten, verified tenant-neutral):** `2026-06-14_001_hosted_brain_postgres.sql` → `2026-06-14_002_harden_hosted_brain_advisors.sql` → `2026-06-14_003_brain_runtime_role.sql` → `2026-06-22_001_durable_oauth_state.sql` → `2026-07-08_001_brain_file_tombstones.sql` → `2026-07-17_001_brain_revision_fts.sql` → `2026-08-19_001_bounded_sync_observability.sql` → `2026-08-22_001_source_reference_identity.sql` → `2026-08-25_001_entra_access_grants.sql` → `20260911113912_resumable_local_ingestion.sql`. Advisors after each and the full security gate after the sequence. Migration 001 auto-creates the private `brain-artifacts` bucket — verify `public=false`, zero object policies. The FTS migration adds a private tombstone-filtered GIN index and no grants; the bounded-observability migration adds the private runtime-only current heartbeat table plus a small partial operational-telemetry index and deletes no history; the source-reference migration adds portable artifact identity fields and a private runtime-only reviewed-link table without invalidating existing source rows; the Entra migration adds tenant-stable access grants and a private append-only authorization audit ledger.
...
PREP    ☑ §1 P0+P1 upstream changes merged  ☑ structural release deployed  ☑ v1.4.3 overlay pushed  ☑ v1.4.5 corrective baseline tagged
        ☑ npm test green  ☑ private ERS-Genomics repo under custody  □ import v1.4.5 + reapply overlay (registry, fly.toml, expectations, runbook)
M1      ☑ Fly org+app reserved (no deploy)  □ Supabase org+project  □ 8 migrations + advisors/security gate  □ brain_runtime login (:6543)
