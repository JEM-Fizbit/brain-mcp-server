...
| Supabase | `brain-platform-pilot` ref `omnwbcdtmtvxasgdmvwr`, "ERSG Prototypes" org (**personal-owned despite the name**), eu-west-2, Pro; PITR off | New ERS org + project; 8 migrations in order (including durable OAuth state, tombstones, private revision FTS, bounded observability, and source-reference identity); `brain_runtime` login on transaction pooler `:6543`; bucket `brain-artifacts`; security re-gate | ~1–2 d |
