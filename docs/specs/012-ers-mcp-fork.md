...
4. **Migrations, in file order (all seven, verified tenant-neutral):** `2026-06-14_001_hosted_brain_postgres.sql` → `2026-06-14_002_harden_hosted_brain_advisors.sql` → `2026-06-14_003_brain_runtime_role.sql` → `2026-06-22_001_durable_oauth_state.sql` → `2026-07-08_001_brain_file_tombstones.sql` → `2026-07-17_001_brain_revision_fts.sql` → `2026-08-19_001_bounded_sync_observability.sql`. Advisors after each and the full security gate after the sequence. Migration 001 auto-creates the private `brain-artifacts` bucket — verify `public=false`, zero object policies. The FTS migration adds a private tombstone-filtered GIN index and no grants; the bounded-observability migration adds the private runtime-only current heartbeat table plus a small partial operational-telemetry index and deletes no history.
...
M1      ☑ Fly org+app reserved (no deploy)  □ Supabase org+project  □ 6 migrations + advisors/security gate  □ brain_runtime login (:6543)
