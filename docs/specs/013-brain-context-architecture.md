...
The first graph-primary observation on 2026-07-18 exposed an independent sync-boundary defect, not a graph-routing regression. A hosted MCP write had created `sources/brand/2026-05-03_ers_brand_identity_guidelines_2025.md` as an ordinary ERS Brain head even though `sources/` is an external ingestion namespace. The local watcher correctly found no such file under the configured Brain vault, but its unchanged conflict was inserted on every five-second cycle, producing 407 duplicate open conflicts. The ERS watcher was paused; a prefix-scoped reconciliation removed only the malformed live head, superseded the 407 duplicates, retained revision history, and preserved the intentional `ip_landscape.md` tombstone. Corrective release `v1.4.4` rejects `sources/`, `inbox/`, and `.brain-sync/` at hosted, filesystem, and local-sync Brain-vault boundaries and deduplicates identical open conflicts transactionally without a schema change. Follow-on `v1.4.5` adds a dry-run-first local state-rebase command that refuses open conflicts, path divergence or hash divergence and backs up the prior state before applying. It was used to remove 19 stale JEM and 73 stale ERS out-of-root tracking entries after exact 39/39 and 50/50 parity proofs. `v1.4.5` (`806117b`) was then deployed successfully; `/health` reported the expected Postgres/Supabase/Postgres storage modes, both hosted Brains remained conflict-free, and 49 JEM plus 60 ERS post-restart watcher cycles reported zero pushes, pulls, conflicts and deletion skips. Graph-primary remains active.
