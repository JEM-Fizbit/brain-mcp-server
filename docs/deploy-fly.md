...
**Verified 26 September 2026:** v1.12.1 (annotated tag object `398f854a14c33c11ed23cff5af4a3c0ad9880118`, public source `c487b18374bacd1e6943327d52d55566f347673a`) deployed successfully using the new Keychain tokens on both owner stacks. JEM completed at 12:52:41 UTC; the separate owner overlay completed at 12:53:54 UTC with its private provenance. Each guarded release passed 590 tests: 579 passed, 11 optional skips, zero failures. Two additional controlled CLI lifecycle tests passed for failed replacement preserving the active credential, verified replacement retaining the previous credential, and retirement refusing absent deployment proof. Those fixtures mock only the Fly/Keychain subprocess boundary; real issuance, Keychain readback, both app scopes, read-only attenuation, cross-Brain refusal and actual deployment were separately verified live.

Both current tokens expire on **25 March 2027**. Repeated Monitor reports show current credentials, passing Fly/hosted health, v1.12.1, fresh sync and zero conflicts. The automation config directories contain no saved account-login token. Existing runtime credentials and profile bindings were preserved. Monitor displays the credential check under Checks; the status command reports exact expiry. Normal operation now needs no Fly browser login; replacement issuance can still require the owner to sign in.
