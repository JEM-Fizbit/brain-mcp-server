...
The cockpit also reports latency in two groups. User-facing read/write operation latency comes from real hosted MCP server tool calls, recorded to Supabase Postgres `brain.sync_events` with event type `hosted_mcp_latency` and metadata source `hosted_mcp_server`. Sync-wait latency comes from hosted smoke/test-drive flows because it measures local-hosted propagation, not a single server handler. The telemetry records tool name, operation kind, safe target metadata, latency, and success/failure state; it does not record file content, patch text, source content, or search query text. `npm run smoke:hosted:oauth` can additionally write client-observed diagnostic rows for all tool calls when `BRAIN_HOSTED_MCP_CLIENT_LATENCY_DB_WRITE=1` is set; `.brain-sync/hosted-mcp-latency.json` is only a fallback cache when Postgres is unavailable or `BRAIN_HOSTED_MCP_LATENCY_CACHE=1` is set. The dashboard reads Postgres first and does not run hidden writes on refresh. It shows latest, average, p50, p95, failed-count, and short trendlines for read/write/sync-wait buckets. Infrastructure latency covers the hosted health request, Postgres summary query, most recent local sync cycle, total doctor run, and each underlying doctor check. These are operator diagnostics, not strict SLOs yet; use them to notice drift before a sync or hosted MCP path becomes painful.
