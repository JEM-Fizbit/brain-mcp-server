...
## Reviewed companion read-back

A completed operator ingestion retains the reviewed Markdown companion in the revision store and the original/extracted text in the source/artifact stores. Hosted source discovery must include the declared companion path even when the provider file ID is opaque. Reading that exact path through `scope="sources"` returns the reviewed companion from the selected Brain's revision store; it does not require exposing original Storage bytes. Existing artifact aliases remain supported, and a companion without a hosted revision returns its metadata rather than inventing content. The ordinary Brain-vault API continues to reject the external `sources/` namespace.

25 September 2026 candidate verification: the first Capture handoff exposed a mismatch between this operator layout and the hosted source reader, which listed an opaque provider ID but omitted the recorded companion. The candidate adds companion discovery and an owner-bound revision read. The full suite passed (560 passes, 11 existing skips), and a read-only check against the selected real personal source returned the exact reviewed companion hash. John approved a personal-only deployment on 25 September. These candidate checks do not substitute for post-deployment hosted acceptance; record that separately after the release. No ERS deployment, schema, permission or credential change is included.

25 September personal hosted acceptance: v1.11.3 is deployed to `jem-brain-mcp` from release commit `0082f8b12f2f83567e6fdac0e2881cde11bb2164`. Authenticated source discovery included the declared companion, and the exact source read matched all 5,851 bytes of the reviewed revision (SHA-256 `6c5928ddb932b77a22d5adbca8ab1e898622276e51a59ad126bdc7843dac4000`). The narrow release branch passed 545 tests with 11 existing skips; main-only local sync changes were excluded. The coordinating Capture item was then read back in `brain_processed` state, version 5, with original custody intact. This proves the selected personal source path, not unattended curation or ERS acceptance. Private source/job receipts remain in the Capture project.
