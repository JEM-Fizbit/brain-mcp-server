...
Hosted auth failures write metadata-only events to the same table with event type `hosted_mcp_auth`, metadata source `hosted_mcp_server`, `timingLayer = "auth"`, and operation kind `auth`. These rows are meant to expose stale/disconnected connector credentials, token expiry, invalid grants, and missing bearer headers. They record sanitized reason codes and HTTP status only; OAuth token failures use the detailed server-side failure class when available, such as `unknown_client_id` or `client_authentication_failed`, while keeping the OAuth error family as the safe target. They do not record access tokens, refresh tokens, authorization headers, request bodies, client secrets, or Brain content. Auth events appear in usage counts and the Operation Log, and they drive a dedicated `hosted_mcp_auth_failures` doctor check, but they remain excluded from latency trendlines and SLO calculations. The `hosted_mcp_auth_failures` check counts failures (`metadata->>'ok' = 'false'`) in a trailing window and returns `warn`/`fail`, so the Checks tab and overall status reflect auth health. It now returns a bounded summary for the cockpit: current-window failures and successes, failure rate, prior-window delta, first/last failure, active-vs-stale state, reason/target/name/HTTP breakdowns, fixed buckets for a trendline, and recent metadata-only failures. Tune the bounded auth read with `BRAIN_HOSTED_MCP_AUTH_EVENT_LIMIT`, `BRAIN_HOSTED_MCP_AUTH_RECENT_LIMIT`, and `BRAIN_HOSTED_MCP_AUTH_BUCKETS`. It shares its window and thresholds with the alerter described next.
