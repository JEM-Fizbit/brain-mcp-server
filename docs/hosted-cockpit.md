...
The cockpit does not run hidden writes or a second doctor just to refresh charts. Under Brain Monitor its once-per-minute poll and its first paint both read the Monitor-owned last-good JSON report. Pressing **Reload** is the deliberate exception: it requests `/api/doctor?fresh=1`, which bypasses that cached report and runs the doctor live, so an operator can clear a transient failure — a cold-connection timeout on one sync cycle, say — without waiting for the next scheduled run. The distinction is hidden versus explicit, not cached versus fresh: nothing re-runs the doctor unless a person asks for it. User-facing latency normally comes from real hosted MCP server tool calls. The hosted server records one latency sample per tool invocation after the handler finishes, including successful and failed read, write, and operational calls. The telemetry write is best-effort and non-blocking by default, so recording a sample should not add response latency. Set `BRAIN_HOSTED_MCP_LATENCY_AWAIT_DB_WRITE=1` only when deliberately diagnosing the telemetry write path itself.

Hosted tool calls write user-facing latency samples to Supabase Postgres `brain.sync_events` with event type `hosted_mcp_latency` and metadata source `hosted_mcp_server`. Server rows use `timingLayer = "server_tool"` and `durationType = "server_tool_handler"`. The telemetry row records tool name, operation kind, safe target metadata such as filename or category, latency, success/failure state, and a bounded DB summary when Postgres work occurred. DB spans record sanitized operation/table names, duration, row count, status, and bounded error text. They do not record SQL text, SQL parameters, file content, patch text, source content, or search query text.

From telemetry `version: 4` (spec 019 phase 2) a span also carries `kind` (`query` or `acquire`) and `startOffsetMs`, the offset from the start of the tool handler. Connection acquisition is measured as its own `acquire` span instead of being billed to whichever SQL statement happened to trigger it — node-postgres implements `Pool.query` as connect-then-query, so a span previously read as "this table was slow" when the real cost was a TCP/TLS/SCRAM handshake or a wait for a free client. Acquire spans use targets `connection.new` and `connection.pooled`, and are recorded only when they cost something (a new connection, a measurable wait, or an error), so the bounded span budget is not spent on sub-millisecond no-ops. The DB summary gains `wallMs` (the union of span intervals — the database-attributable critical path, which cannot exceed the handler duration), alongside the existing `totalMs` (the sum, which double-counts concurrency), plus `acquireCount`, `acquireMs` and `newConnections`. `queryCount` still counts SQL statements only. Rows written before version 4 have no acquire spans and no `startOffsetMs`; they are read as sequential query spans, which is what they were.

Acquire spans participate in the DB span SLO on the same terms as query spans, because the latency is real and a user waits on it. What changes is that the slowest-target finding now names `connection.new` rather than blaming a table for a handshake. The cockpit reads server-emitted Postgres rows first when `BRAIN_REVISION_DATABASE_URL` is configured.
...
The installed native Monitor was rebuilt from this release and restarted with the existing two owner profiles. Identity, paths and runtime credentials were compared unchanged. All old workers exited before the new supervisors started. Both profiles subsequently reported current PID-bound successful cycles, zero recovery attempts and zero conflicts; the Cockpit showed **Safe to use hosted**, no required action and deployed **v1.12.0** for each. Hosted read-only lint completed on both; the personal source-companion abort is resolved. Ordinary content-maintenance findings remain separate from operational readiness. Sleep/freeze/outage/exhaustion behavior was proven with controlled subprocess fixtures; macOS notification delivery and the cause of the original September 24 stall are not established by these checks.
...
Hosted reviewed source companions under `sources/` share revision storage but remain outside vault-sync custody. The worker skips those heads (including tombstones), reports `excludedSourceFiles` in its health/log summary, and continues syncing vault Markdown. It does not copy them into `brain/sources/`, delete operator originals, or alter hosted source revisions. Other reserved namespaces and unsafe vault paths remain refused. Vault listings also exclude these companions in v1.12.0, allowing hosted lint to complete without treating archived sources as vault files. A restart itself does not change source custody.
...
`sync_health` applies the same error classes to the watcher's last completed attempt. The first two fresh transient failures are warnings; three distinct failed observations escalate to failure. Re-reading the same timestamp never increments the count, including after the bounded history rolls over. The watcher exits on an error and Monitor supervises its restart, so cycle numbers alone are not identities. A success, missing history, an observation gap longer than the freshness window, or a later error at watch cycle greater than one resets recurrence; that later cycle proves an intervening success even if the doctor missed it. These are observed failed attempts, not a complete failure-rate measurement. Durable and unknown errors still fail immediately; protected-sync guards remain warnings. Stale transient samples report unknown current health and stay warnings, with heartbeat/supervisor checks assessed separately. Actions show the error class, observation time and recurrence count; Checks retain the source cycle. No extra watcher or doctor-triggered sync retry is introduced.

A bounded verdict history is written beside the report at `hosted-doctor-history.json` (`BRAIN_DOCTOR_HISTORY_FILE`, `BRAIN_DOCTOR_HISTORY_LIMIT`, default `40` cycles), recording brain id, timestamp, overall status, per-check status, and the last sync observation timestamp/status/failed-attempt count. No error text, paths or Brain content is added to history. It exists so a one-off can be told from a pattern: `hosted-doctor.out.json` is overwritten every cycle, so a failing snapshot used to be gone before it could be examined. Persisting it is best-effort and never gates the verdict.
