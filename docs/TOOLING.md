...
`scripts/verify-isolated-resilience.mjs` is **local-state mutating** and opt-in; it is not a routine docs check and is not included in `npm test`. Build first under Node 22. Set `BRAIN_ISOLATED_ADMIN_URL` to a disposable local PostgreSQL cluster with the fixture administrator `brain_acceptance`, literal host `127.0.0.1`, and the cluster's selected port. It must have the `anon` and `authenticated` fixture roles used by the migrations. `BRAIN_ISOLATED_PG_BIN` optionally selects the directory containing matching `pg_dump` and `pg_restore` binaries; the default is Homebrew PostgreSQL 17.

Run `BRAIN_LOAD_LOCAL_ENV=0 node scripts/verify-isolated-resilience.mjs` with that explicit fixture URL. Do not use a production profile or forward a hosted database to the loopback port. The script creates uniquely named disposable databases, applies migrations, performs synthetic authenticated HTTP writes and local sync, dumps/restores synthetic data, and drops only its generated databases in `finally`. It removes its own synthetic files and emits a JSON evidence summary. The caller owns starting/stopping the fixture cluster. Provider backups, Storage permissions, native app installation and production writes are outside this check. See [evidence and limits](production-engineering-followthrough.md#resilience-and-workload-verification).

## Resumable local ingestion (spec 022)

`sources:ingest:local` builds and runs `scripts/ingest-local-source.mjs`. `dry-run` is a safe local source read; `status` is an owner-bound database read with optional local export. `prepare`/`resume` write private jobs, immutable Storage originals and extraction checkpoints. `review`/`approve` write only job review state; `apply` atomically writes approved Brain revisions, source metadata and a receipt. These are explicit operator commands, never routine health checks. The new job-table migration is separately administration-affecting. [Runbook](local-source-ingestion.md).
