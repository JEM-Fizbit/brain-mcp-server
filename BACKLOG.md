...
- Urgent: design a Brain Sync menu-bar app, but first complete an architecture simplification review. Challenge whether the current local OneDrive + local helper + LaunchAgent + cockpit/doctor + Supabase model can be consolidated into a more portable operator architecture for ERS colleagues, including Windows/PC users, before productizing the macOS helper UX.
