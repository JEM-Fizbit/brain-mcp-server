...
- Urgent: implement a consolidated John/operator Brain Sync menu-bar app as the next pilot stability/usability slice, using `docs/specs/006-brain-sync-architecture-simplification.md` as the boundary: reduce local operator overhead now, but do not make the macOS helper the ERS colleague rollout default until the human surface decision is settled.
